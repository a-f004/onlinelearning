...
import cv2 as cv
from cv2 import CAP_PROP_OPENNI_REGISTRATION
import numpy as np
...
subject = "〇〇"
videos = glob.glob("input/" + subject + "/*")

def get_args():
    parser = argparse.ArgumentParser()

    parser.add_argument("--device", type=int, default=0)
    parser.add_argument("--fps", help='cap fps', type=int, default=30)
...
def main():
    ...
    for video in tqdm(videos):
        ...
        size = (cap_width, cap_height)  # 動画の画面サイズ
        video = cv.VideoWriter('output/out.mp4', fmt, frame_rate, size)  # ライター作成
        print("MP4キャプチャの準備が完了しました")
        ...
        while True:
            ...
            if results.multi_face_landmarks is not None:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    # 描画
                    debug_image, coordinates = draw_landmarks(
                        debug_image,
                        face_landmarks,
                        refine_landmarks,
                        left_eye,
            ...
            # csv出力 #################################################
            if firstLoop == True:
                # workbook = openpyxl.load_workbook("output/Sample.xlsx")
                worksheet = workbook.create_sheet(title = str(i))
                worksheet["A1"] = "Time"
                worksheet["B1"] = "Left_iris_x"
                worksheet["C1"] = "Left_iris_y"
                worksheet["D1"] = "Right_iris_x"
                worksheet["E1"] = "Right_iris_y"
                worksheet["F1"] = "Inner_corners_of_eye_x"
                worksheet["G1"] = "Inner_corners_of_eye_y"
                workbook.save("output/" + subject + ".xlsx")
                
            workbook = openpyxl.load_workbook("output/" + subject + ".xlsx")
            # worksheet = workbook[f'{i}'] 
            worksheet = workbook[str(i)] 
            maxrow = worksheet.max_row
            worksheet.cell(maxrow + 1, 1).value = coordinates[0]
            worksheet.cell(maxrow + 1, 2).value = coordinates[1]
            worksheet.cell(maxrow + 1, 3).value = coordinates[2]
            worksheet.cell(maxrow + 1, 4).value = coordinates[3]
            worksheet.cell(maxrow + 1, 5).value = coordinates[4]
            worksheet.cell(maxrow + 1, 6).value = coordinates[5]
            worksheet.cell(maxrow + 1, 7).value = coordinates[6]
            workbook.save("output/" + subject + ".xlsx")
                    
            # MP4キャプチャ書き出し　########################################################
            video.write(debug_image)
            ...
            # 画面反映 #############################################################
            cv.imshow('MediaPipe Face Mesh', debug_image)
            firstLoop = False

        video.release()  # キャプチャファイルを閉じる
        cap.release()
...
def draw_landmarks(image, landmarks, refine_landmarks, left_eye, right_eye, timeStamp):
    image_width, image_height = image.shape[1], image.shape[0]
    ...
    for index, landmark in enumerate(landmarks.landmark):
        ...
        cv.circle(image, (landmark_x, landmark_y), 1, (0, 255, 0), 1)  # Facial landmarksプロット
    ...
    if len(landmark_point) > 0:
        ...
        if refine_landmarks:
            ...
            cv.circle(image, landmark_point[463], 2, (0, 0, 255), -1)
            # 左目：目頭側
        ...
        #   虹彩座標の格納  
            left_iris_x = landmark_point[468][0]
            left_iris_y = landmark_point[468][1]
            right_iris_x = landmark_point[473][0]
            right_iris_y = landmark_point[473][1]
            
        #  目頭作業の格納
            inner_corners_of_eye_x = (landmark_point[463][0] + landmark_point[243][0]) / 2
            inner_corners_of_eye_y = (landmark_point[463][1] + landmark_point[243][1]) / 2

    coordinates.extend([round(timeStamp, 3), left_iris_x, left_iris_y, right_iris_x, right_iris_y, inner_corners_of_eye_x, inner_corners_of_eye_y])
    print(coordinates)
